 ...
 */
function pmprogpg_activation() {
        wp_schedule_event(current_time('timestamp'), 'daily', 'pmprogpg_cancel_overdue_orders');
        wp_schedule_event(current_time('timestamp')+1, 'daily', 'pmprogpg_recurring_orders');
        wp_schedule_event(current_time('timestamp')+2, 'daily', 'pmprogpg_reminder_emails');

        do_action('pmprogpg_activation');
}
register_activation_hook( PMPRO_GENERIC_GATEWAY_BASE_FILE, 'pmprogpg_activation' );

/**
 * Clear crons on plugin deactivation.
 */
function pmprogpg_deactivation() {
        wp_clear_scheduled_hook('pmprogpg_cancel_overdue_orders');
        wp_clear_scheduled_hook('pmprogpg_recurring_orders');
        wp_clear_scheduled_hook('pmprogpg_reminder_emails');

        do_action('pmprogpg_deactivation');
}
register_deactivation_hook( PMPRO_GENERIC_GATEWAY_BASE_FILE, 'pmprogpg_deactivation' );

/**
 * Add GPG crons to the PMPro crons list to pull the site health report.
 *
 ...
 * @since 1.1.3
 */
function pmprogpg_add_cron_schedule( $schedules ) {
        $schedules['pmprogpg_cancel_overdue_orders'] = array(
                'interval' => 'daily',
        );
        $schedules['pmprogpg_recurring_orders'] = array(
                'interval' => 'daily',
        );
        $schedules['pmprogpg_reminder_emails'] = array(
                'interval' => 'daily',
        );
        return $schedules;
}

add_filter( 'pmpro_registered_crons', 'pmprogpg_add_cron_schedule' );

/**
 * Create pending orders for subscriptions.
 */
function pmprogpg_recurring_orders() {
        global $wpdb;

        // If the PMPro_Subscriptions class doesn't exist, use the legacy logic.
        if ( ! class_exists( 'PMPro_Subscription' ) ) {
                pmprogpg_recurring_orders_legacy();
                return;
        }
        ...
        foreach ( $levels as $level ) {
                ...
                foreach ( $subscriptions as $subscription_id ) {
                        ...
                        $pending_order->gateway = 'generic';
                        $pending_order->payment_type = 'Generic';
                        ...
                        // Send a generic_pending email.
                        pmprogpg_send_generic_pending_email( $pending_order );
                }
        }
}
add_action('pmprogpg_recurring_orders', 'pmprogpg_recurring_orders');

/*
        Send reminder emails for pending invoices.
*/
function pmprogpg_reminder_emails() {
        global $wpdb;
        ...
        foreach ( $levels as $level ) {
                ...
                foreach ( $orders as $order_id ) {
                        ...
                        // Send email.
                        pmprogpg_send_generic_pending_reminder_email( $order );
                }
        }
}
add_action('pmprogpg_reminder_emails', 'pmprogpg_reminder_emails');

/**
 * Cancel overdue members.
 */
function pmprogpg_cancel_overdue_orders() {
        global $wpdb;
        ...
        foreach( $levels as $level ) {
                ...
                foreach ( $orders as $order_id ) {
                        ...
                        // Update the order.
                        $order->status = 'error';
                        $order->notes .= "Generic Payment Cancelled:" . date( 'Y-m-d' ) . "\n";
                        $order->saveOrder();
...
add_action('pmprogpg_cancel_overdue_orders', 'pmprogpg_cancel_overdue_orders');
 ...
 */
function pmprogpg_recurring_orders_legacy() {
        global $wpdb;
...
// ADDED Extra brackets round OR as sql results were missing some orders (match for same user id was not being used due to the missing brackets) 
                        */
                        ...
                        foreach($orders as $order_id)
                        {
                                ...
                                $morder->gateway = "generic";
                                $morder->setGateway();
                                $morder->payment_type = "Generic";
                                ...
                                // Send email.
                                pmprogpg_send_generic_pending_email( $morder );
                        }
                }
        }
}
