 ...
 *
 * This is because in 3.0.3, we have the ability to overwrite the core Generic gateway class. With that overwritten class,
 ...
 * is being implemented in the 1.0 release of GPG.
 *
 * @since 1.0
 */
function pmprogpg_add_member_pending_actions() {
        // If running PMPro v3.0.3+, return.
        if ( ! defined( 'PMPRO_VERSION') || version_compare( PMPRO_VERSION, '3.0.3', '>=' ) ) {
                return;
        }

        add_filter( "pmpro_has_membership_access_filter", "pmprogpg_pmpro_has_membership_access_filter", 10, 4 );
        add_filter( 'pmpro_member_shortcode_access', 'pmprogpg_pmpro_member_shortcode_access', 10, 4 );
        add_filter( 'pmpro_non_member_text_filter', 'pmprogpg_generic_pending_lock_text' );
}
add_action( 'init', 'pmprogpg_add_member_pending_actions' );

/**
 * Check if a member's status is still pending, i.e. they haven't made their first generic payment.
 *
 * @since .5
 *
 ...
 */
function pmprogpg_isMemberPending($user_id, $level_id = 0)
{
        global $pmprogpg_pending_member_cache;

        //check the cache first
        if(isset($pmprogpg_pending_member_cache) && 
           isset($pmprogpg_pending_member_cache[$user_id]) && 
           isset($pmprogpg_pending_member_cache[$user_id][$level_id]))
                return $pmprogpg_pending_member_cache[$user_id][$level_id];
        
        //make room for this user's data in the cache
        if(!is_array($pmprogpg_pending_member_cache)) {
                $pmprogpg_pending_member_cache = array();
        } elseif(!is_array($pmprogpg_pending_member_cache[$user_id])) {
                $pmprogpg_pending_member_cache[$user_id] = array();
        }       
        $pmprogpg_pending_member_cache[$user_id][$level_id] = false;
        ...
        if ( empty( $level_id ) ) {
                ...
                if ( ! empty( $levels) ) {
                        foreach ( $levels as $level ) {
                                if ( pmprogpg_isMemberPending( $user_id, $level->id ) ) {
                                        $is_pending = true;
                                }
                        }
                }
                $pmprogpg_pending_member_cache[$user_id][$level_id] = $is_pending;
                return $is_pending;
        }
        ...
        if(!empty($order->status))
        {
                if($order->status == "pending")
                {
                        ...
                        if(!empty($paid_order) && !empty($paid_order->id) && $paid_order->gateway === 'generic')
                        {
                                //how long ago is too long?
                                $options = pmprogpg_getOptions($membership_level->id);
                                ...
                                if($paid_order->timestamp < $cutoff)
                                        $pmprogpg_pending_member_cache[$user_id][$level_id] = true;
                                else
                                        $pmprogpg_pending_member_cache[$user_id][$level_id] = false;
                        }
                        else
...
function pmprobpc_memberHasAccessWithAnyLevel( $user_id, $content_levels = null ) {
        ...
        foreach ( $content_levels as $content_level ) {
                if ( in_array( $content_level, $user_level_ids ) && ! pmprogpg_isMemberPending( $user_id, $content_level ) ) {
                        return true;
                }
        }
        return false;
 ...
 *
 * @deprecated 1.0 Use pmprogpg_isMemberPending() instead.
 */
function pmprobpc_isMemberPending($user_id) {
        _deprecated_function( __FUNCTION__, '1.0', 'pmprogpg_isMemberPending()' );
        return pmprogpg_isMemberPending($user_id);
}

//if a user's last order is pending status, don't give them access
function pmprogpg_pmpro_has_membership_access_filter( $hasaccess, $mypost, $myuser, $post_membership_levels ) {
 ...
 * @since 0.10
 */
function pmprogpg_pmpro_member_shortcode_access( $hasaccess, $content, $levels, $delay ) {
        global $current_user;
...
/**
 *  Show a different message for users whose payments are pending
 */
function pmprogpg_generic_pending_lock_text( $text ){
        global $current_user;
        ...
        if(pmprogpg_isMemberPending($current_user->ID)==true && pmprogpg_wouldHaveMembershipAccessIfNotPending()==true){
                $text = __("Your payment is currently pending. You will gain access to this page once it is approved.", "pmpro-generic-gateway");
        }
        return $text;
}

function pmprogpg_wouldHaveMembershipAccessIfNotPending($user_id = NULL){
        global $current_user;
        if(!$user_id)
                $user_id = $current_user->ID;
        
        remove_filter("pmpro_has_membership_access_filter", "pmprogpg_pmpro_has_membership_access_filter", 10, 4);
