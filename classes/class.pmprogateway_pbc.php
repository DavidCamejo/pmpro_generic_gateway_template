<?php
class PMProGateway_gpg extends PMProGateway {
        function __construct() {
                $this->gateway = 'generic';
        }
        ...
        function process(&$order) {
                ...
                if ( pmpro_isLevelRecurring( $checkout_level ) ) {
                        $order->subscription_transaction_id = 'GENERIC' . $order->code;
                }
                ...
                update_pmpro_membership_order_meta( $order->id, 'gpg_profile_start_date', pmpro_calculate_profile_start_date( $order, 'Y-m-d H:i:s' ) );

                // If this order was free, then we don't need to wait for the payment to be recieved.
                ...
                // The order is paid, so we need to wait for a payment to be recieved.
                // Save the checkout data into the order.
                pmpro_save_checkout_data_to_order( $order );

                // Send the generic_pending emails.
                pmprogpg_send_generic_pending_email( $order );
                pmprogpg_send_generic_pending_admin_email( $order );
        ...
        public function update_subscription_info( $subscription ) {
                ...
                if ( ! empty( $pending_orders ) ) {
                        ...
                        if ( ! empty( $newest_orders ) ) {
                                ...
                                // Check if the most recent order has gpg_profile_start_date meta set.
                                $gpg_profile_start_date = get_pmpro_membership_order_meta( $newest_order->id, 'gpg_profile_start_date', true );
                                if ( ! empty( $gpg_profile_start_date ) ) {
                                        // Set the next payment date to the profile start date.
                                        $update_array['next_payment_date'] = $gpg_profile_start_date;
                                } else {
